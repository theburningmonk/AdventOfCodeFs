...
let people = 
    input.Split '\n'
    |> Seq.map (fun l -> l.Split ' ' |> Seq.head)
    |> Seq.distinct
...
let input = 
    ...
    |> Seq.collect (fun p -> 
        [
            sprintf "Yan would gain 0 happiness units by sitting next to %s.\n" p
            sprintf "%s would gain 0 happiness units by sitting next to Yan.\n" p
        ])
    |> Seq.fold (+) ""
    |> fun ls -> ls + input
